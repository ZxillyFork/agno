...
def _stamp_approval_id_on_tools(
    ...
    approval_id: str,
    replace_approval_id: Optional[str] = None,
) -> None:
    """Stamp approval_id on active approval tools for the current pause."""
    if tools:
        for tool in tools:
            current_id = getattr(tool, "approval_id", None)
            if _is_active_approval_tool(tool) and (current_id is None or current_id == replace_approval_id):
...
def create_approval_from_pause(
    ...
    try:
        ...
        _stamp_approval_id_on_tools(tools, requirements, approval_id, replace_approval_id=existing_approval_id)
...
def _collect_active_approval_tools(run_response: Any) -> List[Any]:
    active_tools = [tool for tool in _collect_all_approval_tools(run_response) if _is_active_approval_tool(tool)]
    bound_tools = [tool for tool in active_tools if getattr(tool, "approval_id", None) is not None]
    return bound_tools or active_tools
...
def _group_tools_by_approval(db: Any, run_id: str, run_response: Any, tools: List[Any]) -> List[tuple]:
    ...
    for tool in tools:
        ...
        if aid:
            owner_run_id = mid or run_id
            key = f"approval:{owner_run_id}:{aid}"
            if key not in cache:
                cache[key] = _get_approval_for_run(db, owner_run_id, approval_id=aid)
            record = cache[key]
        if record is None:
            if mid:
                key = f"run:{mid}"
...
async def _agroup_tools_by_approval(db: Any, run_id: str, run_response: Any, tools: List[Any]) -> List[tuple]:
    ...
    for tool in tools:
        ...
        if aid and get_one is not None:
            ...
            key = f"approval:{owner_run_id}:{aid}"
            if key not in cache:
                cache[key] = await _aget_approval_for_run(db, owner_run_id, approval_id=aid)
            record = cache[key]
        if record is None:
            if mid:
                key = f"run:{mid}"
...
def check_and_apply_approval_resolution(db: Any, run_id: str, run_response: Any) -> None:
    ...
    approval_tools = _collect_active_approval_tools(run_response)
    if not approval_tools:
        return

    pairs = _group_tools_by_approval(db, run_id, run_response, approval_tools)
    ...
    if any(
        not _has_usable_approval_resolution(
            tool, (record or {}).get("status", "pending"), (record or {}).get("resolution_data")
        )
...
async def acheck_and_apply_approval_resolution(db: Any, run_id: str, run_response: Any) -> None:
    ...
    pairs = await _agroup_tools_by_approval(db, run_id, run_response, approval_tools)
    if any(record is None for _, record in pairs):
        raise RuntimeError(
            "No approval record found for this run. Cannot continue a run that requires external approval."
        )
